...
static void MX_I2C1_Init(void)
{
  ...
  */
  ...
  LL_DMA_SetPeriphAddress(DMA1, LL_DMA_CHANNEL_1, LL_I2C_DMA_GetRegAddr(I2C1, LL_I2C_DMA_REG_DATA_TRANSMIT));
...
/**********************************************************************
  ...
  * @retval None
  ********************************************************************/
s16	FLT_Kalman(s16 val)
{
	static s16 K = 1000 *  0.95;		//0.5
	static s16 prev_val = 0;
	s16	result;

	result = (((K*val)+((1000-K)*prev_val))/1000);
	prev_val = val;
	return result;
}
